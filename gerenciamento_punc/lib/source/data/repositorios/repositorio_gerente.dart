...
import '../modelos/trajetoria.dart';
...
class RepositorioGerente {
  ...
  /// GET /api/gerente/trajetorias
  Future<List<TrajetoriaGerente>> listarTrajetorias({
    required int idGerente,
  }) async {
    final resposta = await _client.get(
      '/api/gerente/trajetorias',
      corpo: {'id_gerente': idGerente},
    );

    final lista = resposta['trajetorias'] as List<dynamic>? ?? [];
    return lista
        .map(
          (item) => TrajetoriaGerente.fromJson(item as Map<String, dynamic>),
        )
        .toList();
  }

  /// GET /api/gerente/trajetorias/:id/localizacoes
  Future<List<LocalizacaoTrajetoria>> listarLocalizacoesTrajetoria({
    required int idGerente,
    required int idTrajetoria,
  }) async {
    final resposta = await _client.get(
      '/api/gerente/trajetorias/$idTrajetoria/localizacoes',
      corpo: {'id_gerente': idGerente},
    );

    final lista = resposta['localizacoes'] as List<dynamic>? ?? [];
    return lista.asMap().entries.map((entry) {
      return LocalizacaoTrajetoria.fromJson(
